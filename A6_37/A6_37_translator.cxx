...
sym* symtable::lookup(string name)                                                 // Lookup an symbol in the symbol table, whether it exists or not
{
    ...
    if(ST == this and !ptr){
        ...
        if(ST==globalST)symbol->category="global";
...
bool compareByName(const sym &a, const sym &b)
{
...
void symtable::update()                                                                            // Update the symbol table and the offsets in it
{
    ...
    table.sort(compareByName);                                                                           // sort the table according to the offsets of the elements));
    ...
    for (auto entry : this->table)
    {
        if (entry.category == "param")
        {
            if (entry.size != 0)
            {
                activationRecord->totalDisplacement -= entry.size;
                activationRecord->displacement[entry.name] = activationRecord->totalDisplacement;
            }
        }
    }
    ...
    for (auto entry : (this)->table)
    {
        if ((entry.category == "local" && entry.name != "return") || entry.category == "temporary")
        {
            if (entry.size != 0)
            {
                activationRecord->totalDisplacement -= entry.size;
...
void symtable::print()                                                                                
{
    ...
    cout<<"Category";                                                                            // Category of the symbol table entry
    repeatTimes(15, " ");
    ...
    for(list<sym>::iterator it=table.begin(); it!=table.end(); it++) {                                // iterate through all the elements in the symbol table and print their details
        ...
        cout<<it->category;                                                                      // print category of the current symbol entry
        repeatTimes(15-it->category.length()," ");
...
void quad::print() 
{
	...
	//----- Asignment operator --------
	else if(op=="=" || op=="=str" || op=="=char") cout <<res<<" = "<<arg1 ;	
	...
	else if (op == "func") std::cout << "Function start " << res << ": ";
    else if(op=="funcend") cout << "Function end " << res;
...
void quadArray::print()                                                                                // print the quad Array i.e the list of TAC
{
    ...
    while(it!=Array.end()) 
    {
        ...
        // if(it->op=="func")                                                                             // print the label if it is the operator 
        // {
        //     cout<<endl<<j<<": ";
        //     it->print();
        // }
...
{   
    ...
    string tmp_name = "t"+convertIntToString(newCount++);                                              // generate name of temporary variable
