...
identifier-aug : IDENTIFIER{
	...
	// cout << "identifier-aug" << $1->name << " " << var_type<< endl;
...
declaration:
    type-specifier init-declarator SEMICOLON{ 
		// CHANGED
		// ST->lookup1($2->name)->update(new symboltype(var_type));        //lookup the name in the symbol table and update its type
		// sym* symbol = new sym($2->name);
        // ST->table.push_back(*symbol);                                                                  // push the symbol into the table
        // return &table.back();     
		// ST->lookup($2->name)->update(new symboltype("int"));        //lookup the name in the symbol table and update its type
	}
;
...
|   IDENTIFIER SQUARE_BRACKET_OPEN INTEGER_CONSTANT SQUARE_BRACKET_CLOSE {
		...
		if(prev==NULL) 
		{
			symboltype* s = new symboltype("arr", $1->type);    //get initial value
			$$ = $1->update(s);
		}
		else 
		{
...
iteration-statement:
	FOR F ROUND_BRACKET_OPEN X changetable declaration M expression-statement M expressionopt N ROUND_BRACKET_CLOSE M loop-statement     //for loop
	{
	...
	| FOR F ROUND_BRACKET_OPEN X changetable expression-statement M expression-statement M expressionopt N ROUND_BRACKET_CLOSE M loop-statement     //for loop
	{
		$$ = new Statement();		 //create new statement
		convertIntToBool($8);  //convert check expression to boolean
		backpatch($8->truelist, $13);	//if expression is true, go to M2
	...
	| FOR F ROUND_BRACKET_OPEN X changetable declaration M expression-statement M expressionopt N ROUND_BRACKET_CLOSE M CURLY_BRACKET_OPEN block-item-listopt CURLY_BRACKET_CLOSE      //for loop
	{
		$$ = new Statement();		 //create new statement
		convertIntToBool($8);  //convert check expression to boolean
		backpatch($8->truelist, $13);	//if expression is true, go to M2
	...
	| FOR F ROUND_BRACKET_OPEN X changetable expression-statement M expression-statement M expressionopt N ROUND_BRACKET_CLOSE M CURLY_BRACKET_OPEN block-item-listopt CURLY_BRACKET_CLOSE
	{	
		$$ = new Statement();		 //create new statement
		convertIntToBool($8);  //convert check expression to boolean
		backpatch($8->truelist, $13);	//if expression is true, go to M2
...
function-definition:
    type-specifier declarator changetable CURLY_BRACKET_OPEN block-item-listopt CURLY_BRACKET_CLOSE{
		int next_instr=0;	 	
		ST->parent=globalST;
		table_count = 0;
		label_table.clear();
